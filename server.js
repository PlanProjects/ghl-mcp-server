...
const safe = async (fn) => {
  try { const r = await fn(); return { success: true, data: r.data }; }
    ...
    console.error("GHL API error:", e?.response?.status, JSON.stringify(e?.response?.data));
...
async function callTool(name, args = {}) {
  switch (name) {
    ...
    case "ghl_list_pipelines":
      return safe(() => ghl.get("/opportunities/pipelines", { params: { locationId: GHL_LOCATION_ID } }));
    case "ghl_list_opportunities":
      return safe(() => ghl.get("/opportunities/search", { params: { location_id: GHL_LOCATION_ID, pipeline_id: args.pipelineId, status: args.status, limit: args.limit || 20 } }));
    case "ghl_get_opportunity":
      return safe(() => ghl.get(`/opportunities/${args.opportunityId}`));
    case "ghl_create_opportunity":
      return safe(() => ghl.post("/opportunities/", { locationId: GHL_LOCATION_ID, pipelineId: args.pipelineId, pipelineStageId: args.stageId, contactId: args.contactId, name: args.name, monetaryValue: args.monetaryValue, status: args.status || "open" }));
    case "ghl_update_opportunity":
      return safe(() => ghl.put(`/opportunities/${args.opportunityId}`, { pipelineStageId: args.stageId, monetaryValue: args.monetaryValue, status: args.status, name: args.name }));
    case "ghl_delete_opportunity":
      return safe(() => ghl.delete(`/opportunities/${args.opportunityId}`));

    // CONVERSATIONS
    case "ghl_list_conversations":
      return safe(() => ghl.get("/conversations/search", { params: { locationId: GHL_LOCATION_ID, contactId: args.contactId, type: args.type, limit: args.limit || 20, unread: args.unread } }));
    case "ghl_get_conversation":
      return safe(() => ghl.get(`/conversations/${args.conversationId}`));
    case "ghl_send_sms":
      return safe(() => ghl.post("/conversations/messages", { type: "SMS", contactId: args.contactId, message: args.message, locationId: GHL_LOCATION_ID }));
    case "ghl_send_email":
      return safe(() => ghl.post("/conversations/messages", { type: "Email", contactId: args.contactId, subject: args.subject, body: args.body, fromName: args.fromName, fromEmail: args.fromEmail, locationId: GHL_LOCATION_ID }));

    // CALENDARS
    case "ghl_list_calendars":
      return safe(() => ghl.get("/calendars/", { params: { locationId: GHL_LOCATION_ID } }));
    case "ghl_list_appointments":
      return safe(() => ghl.get("/calendars/events", { params: { locationId: GHL_LOCATION_ID, calendarId: args.calendarId, startTime: args.startTime, endTime: args.endTime } }));
    case "ghl_create_appointment":
      return safe(() => ghl.post("/calendars/events/appointments", { locationId: GHL_LOCATION_ID, calendarId: args.calendarId, contactId: args.contactId, startTime: args.startTime, title: args.title, appointmentStatus: args.appointmentStatus || "confirmed" }));
    case "ghl_update_appointment":
      return safe(() => ghl.put(`/calendars/events/appointments/${args.appointmentId}`, { startTime: args.startTime, appointmentStatus: args.appointmentStatus }));

    // FORMS
    case "ghl_list_forms":
      return safe(() => ghl.get("/forms/", { params: { locationId: GHL_LOCATION_ID } }));
    case "ghl_get_form_submissions":
      return safe(() => ghl.get("/forms/submissions", { params: { locationId: GHL_LOCATION_ID, formId: args.formId, limit: args.limit || 20 } }));

    // CAMPAIGNS
    case "ghl_list_campaigns":
      return safe(() => ghl.get("/campaigns/", { params: { locationId: GHL_LOCATION_ID, status: "active" } }));
    ...
    case "ghl_list_custom_values":
      return safe(() => ghl.get(`/locations/${GHL_LOCATION_ID}/customValues`));
...
async function handleMCP(body) {
  ...
  if (method === "initialize") {
    return { jsonrpc: "2.0", id, result: { protocolVersion: MCP_VERSION, serverInfo: { name: "ghl-mcp-server", version: "2.0.0" }, capabilities: { tools: {} } } };
  }
...
function sendResponse(req, res, data) {
  ...
  if (accept.includes("text/event-stream")) {
    ...
    if (data === null) return res.status(202).end();
...
app.get("/health", (req, res) => res.json({ status: "ok", service: "ghl-mcp-server", version: "2.0.0", apiVersion: "v2" }));

app.get("/.well-known/oauth-protected-resource", (req, res) => {
  res.json({ resource: BASE_URL, bearer_methods_supported: [] });
});
app.get("/.well-known/oauth-authorization-server", (req, res) => {
  res.json({ issuer: BASE_URL, authorization_endpoint: `${BASE_URL}/authorize`, token_endpoint: `${BASE_URL}/token`, registration_endpoint: `${BASE_URL}/register`, response_types_supported: ["code"], grant_types_supported: ["authorization_code"], code_challenge_methods_supported: ["S256"] });
});
app.post("/register", (req, res) => {
  res.status(201).json({ client_id: `ghl-client-${Date.now()}`, client_secret: "not-used", client_id_issued_at: Math.floor(Date.now() / 1000), grant_types: ["authorization_code"], redirect_uris: req.body?.redirect_uris || [], response_types: ["code"], token_endpoint_auth_method: "none" });
});
app.get("/authorize", (req, res) => {
  const { redirect_uri, state } = req.query;
  const url = new URL(redirect_uri);
  url.searchParams.set("code", `auth-code-${Date.now()}`);
  if (state) url.searchParams.set("state", state);
  res.redirect(url.toString());
});
app.post("/token", (req, res) => {
  res.json({ access_token: `ghl-token-${Date.now()}`, token_type: "Bearer", expires_in: 86400, scope: "mcp" });
});
...
app.listen(PORT, () => {
  console.log(`✅ GHL MCP Server v2 running on port ${PORT}`);
  console.log(`📡 Using GHL API v2 (services.leadconnectorhq.com)`);
});
